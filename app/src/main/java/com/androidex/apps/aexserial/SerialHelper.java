...
public abstract class SerialHelper {
    ...
    */
    ...
    public int open() {
        if(mSerialFd > 0) {
            log(String.format("已经打开%s：%d", sPort, mSerialFd));
            close();
        }
    ...
    public void startReadSerial(){
        Runnable run=new Runnable() {
            public void run() {
                log("开始读取串口数据");
                /*while(mSerialFd > 0) {
                    byte[] r = serial.serial_read(mSerialFd, 100, 10000000);
                    if (r != null) {
                ...
                */
                serial.native_serial_readloop(mSerialFd,100);
                log("读取结束");
            }
        };
